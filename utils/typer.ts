...
class TyperText {
    ...
    constructor(options: Options) {
        ...
        if (this.startIndex < 0) {
            this.handleError(
                `startIndex ${options.startIndex} not allow smaller than 0`
            );
            return;
        }
        ...
        if (this.startIndex > this.text.length) {
            this.handleError(
                `startIndex ${options.startIndex} not allow larger than text length ${this.text.length}`
            );
            return;
        }

        this.className =
            "auto-input-text-" + Math.random().toString(36).slice(-8);
        this.el.className += this.className;
        this.el.setAttribute(
            "style",
            `color: ${this.color};font-size: ${this.fontSize}`
        );
    ...
    private initWords() {
        ...
        this.el.innerHTML = `<span>${this.words
    ...
    render() {
        ...
        const timer = setInterval(() => {
            const str = this.words
                .slice(this.startIndex, index + this.startIndex)
                .join("");
    ...
    private addStyle() {
        ...
        if (sheet) {
            sheet.insertRule(
                `.${this.className} span {position: relative;}`,
                0
            );
            sheet.insertRule(
                `@keyframes shadow { from { opacity: 1; } to { opacity: 0; }}`,
                1
            );
            sheet.insertRule(
                `.${
                    this.className
                } span::after {content: "";position: absolute;bottom: ${this.calSize(
                    this.fontSize,
                    5
                )};width: ${this.calSize(
                    this.fontSize
    ...
    private calSize(value: string, scale = 1.5) {
        const size = parseInt((value.match(/[0-9]*/g) as string[])[0]) / scale,
            unit =
                (value.match(/[a-z]*/g) as string[]).find(
                    (item: string) => item
                ) || "px";
